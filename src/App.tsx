...
const isArticlePublished = (article: Article) =>
  new Date(article.publicationDate) <= new Date();
const isDraft = (article: Article) =>
  new Date(article.publicationDate).getFullYear() === 2099;
...
const CategoryDetailView: React.FC<{
  ...
  const sections = useMemo(() => {
    const processArticles = (articles: Article[]) => {
      ...
      const scheduled = uniqueArticles.filter(
        (a) => !isArticlePublished(a) && !isDraft(a)
      );
    ...
    if (isAllView) {
      const allCategoryArticles = category.subcategories.flatMap(
        (sub) => sub.articles
      );
      const { publishedArticles, unpublishedArticles } =
        processArticles(allCategoryArticles);

      if (publishedArticles.length === 0 && unpublishedArticles.length === 0)
        return [];
    ...
    return category.subcategories
      ...
      .map((sub) => {
        const { publishedArticles, unpublishedArticles } = processArticles(
          sub.articles
        );
        return {
          name: sub.name,
  ...
  return (
    <section
      ...
      className="mt-8 md:mt-12 scroll-mt-20"
    >
        ...
        {sections.map((section) => {
          ...
          return (
            <div key={section.name}>
              ...
              {section.unpublishedArticles.length > 0 && (
                <div
                  className={`p-6 rounded-xl bg-[#0f323e]/80 border border-white/10 ${
                    section.publishedArticles.length > 0 ? "mt-8" : ""
                  }`}
                >
                  ...
                  <ul className="list-none space-y-3">
                    {section.unpublishedArticles.map((article) => (
                      <li
                        key={article.id}
                        className="text-gray-400 flex items-start justify-between"
                      >
                        <div className="flex items-start">
                          <svg
                            className="w-4 h-4 mr-3 mt-1 text-[#308271] flex-shrink-0"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
  ...
  <div className="fixed bottom-4 right-4 z-50 bg-[#0f323e]/80 backdrop-blur-lg p-3 rounded-lg shadow-2xl border border-white/20 flex items-center space-x-4">
    <div className="flex items-center space-x-2">
      <span
        className={`text-sm font-medium ${
          !isEditMode ? "text-white" : "text-gray-400"
        }`}
      >
        Preview
      </span>
      <button
        ...
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
          isEditMode ? "bg-[#308271]" : "bg-gray-600"
        }`}
      >
        <span
    ...
    {isEditMode && (
      <div className="flex items-center space-x-2 border-l border-gray-600 pl-4">
        <button
          onClick={onCreate}
          className="text-xs text-gray-300 hover:text-white transition-colors border border-gray-600 px-2 py-1 rounded-md hover:bg-[#308271] hover:border-[#308271]"
        >
          Create
        </button>
        <button
          onClick={onCycleFeatured}
          className="text-xs text-gray-300 hover:text-white transition-colors border border-gray-600 px-2 py-1 rounded-md hover:bg-[#308271] hover:border-[#308271]"
        >
          Cycle Featured
        </button>
        ...
        <div className="flex items-center space-x-1 border-l border-gray-600 pl-2 ml-2">
          <button
            ...
            className="text-xs text-gray-300 hover:text-white transition-colors border border-gray-600 px-2 py-1 rounded-md hover:bg-blue-600 hover:border-blue-600"
          >
            Backup
          </button>
          <button
        ...
        <div className="flex items-center space-x-1 border-l border-gray-600 pl-2 ml-2">
          ...
          <button
            onClick={onReset}
            className="text-xs text-gray-300 hover:text-red-400 transition-colors border border-gray-600 px-2 py-1 rounded-md hover:border-red-900"
          >
            Reset
          </button>
        </div>
...
const FeaturedArticleCard: React.FC<{
  ...
  onSelectArticle: (article: Article) => void;
}> = ({ article, isEditMode, onEdit, onSelectArticle }) => {
  return (
    <div className="relative rounded-xl overflow-hidden bg-[#0f323e] shadow-2xl mb-12">
      <div className="md:flex">
        ...
        <div className="p-8 md:p-12 md:w-1/2 flex flex-col justify-center">
          <p className="text-sm font-semibold text-[#308271] uppercase tracking-wide">
            Featured Article
          </p>
          <h2 className="mt-2 text-3xl font-bold text-white leading-tight">
            {article.title}
          </h2>
          <p className="mt-4 text-gray-300">{article.description}</p>
          <button
            onClick={() => !isEditMode && onSelectArticle(article)}
            className={`mt-6 inline-block bg-[#308271] text-white px-6 py-3 rounded-md font-semibold hover:bg-[#45a08d] transition-colors self-start ${
...
const App: React.FC = () => {
  ...
  const { featuredArticle, recentArticles } = useMemo(() => {
    const allArticles = blogData.flatMap((cat) =>
    ...
    const uniquePublished = Array.from(
      new Map(allPublished.map((a) => [a.id, a])).values()
    );
    uniquePublished.sort(
      (a, b) =>
    ...
    const featured =
      uniquePublished.find((a) => a.isFeatured) || uniquePublished[0] || null;
  ...
  const relatedArticles = useMemo(() => {
    ...
    const allCategoryArticles = parentCategory.subcategories
      ...
      .filter((a) => a.id !== selectedArticle.id && isArticlePublished(a));

    const uniqueArticles = Array.from(
      new Map(allCategoryArticles.map((a) => [a.id, a])).values()
    );

    // Simple shuffle and take 2-3
    return uniqueArticles.sort(() => 0.5 - Math.random()).slice(0, 3);
  ...
  return (
    <div
      style={appStyle}
      className="min-h-screen w-full text-white font-sans flex flex-col"
    >
      <Header
